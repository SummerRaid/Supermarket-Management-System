...
public abstract class BaseDAO<T> {
    ...
    protected void close(ResultSet rs , PreparedStatement psmt){
        try {
            if (rs != null) {
                rs.close();
            }
    ...
    private void setValue(Object obj ,  String property , Object propertyValue) throws Exception {
        ...
        if(field!=null){
            ...
            if(isMyType(typeName)) {
                ...
                //propertyValue = this.load("select * from t_" + field.getName() + " where id = ?", propertyValue);
                Constructor constructor = typeNameClass.getDeclaredConstructor(Integer.class);
                propertyValue = constructor.newInstance(propertyValue);
            }
            if("java.util.Date".equals(typeName)) {
                propertyValue = StringUtil.localDateTimeToDate((LocalDateTime) propertyValue);
            }
    ...
    private static boolean isNotMyType(String typeName) {
        return "java.lang.Integer".equals(typeName)
                ...
                || LocalDateTime.class.getName().equals(typeName);
    }
    ...
    protected List<T> executeQuery(String sql , Object... params){
        ...
        try {
            ...
            while(rs.next()){
                ...
                for(int i = 0 ; i<columnCount;i++){
                    String columnName;
                    if(StringUtil.isNotEmpty(rsmd.getColumnLabel(i+1))) {
                        columnName = rsmd.getColumnLabel(i+1);
                    } else {
                        columnName = rsmd.getColumnName(i+1);            //fid   fname   price
                    }
                    Object columnValue = rs.getObject(i+1);     //33    苹果      5
                    setValue(entity,columnName,columnValue);
                }
